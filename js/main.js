...
els.productList.addEventListener("click", (e) => {
  //lee los clics dentro de las tarjetas de los productos
...
els.cartRows.addEventListener("click", (e) => {
  ...
  if (plus) {
    increaseItem(plus.dataset.id);
  }
  // regresa una unidad y devuelve el stock
  if (minus) {
    decreaseItem(minus.dataset.id);
  }
...
function renderCart() {
  ...
  if (cart.size === 0) {
    ...
    for (const it of cart.values()) {
      //por cada item de carrito genera una fila con nombre, controles, subtotal y botón de eliminar
      rows.push(`
        <tr>
          <td>${it.nombre}</td>
          <td class="text-center">
            <div class="btn-group btn-group-sm" role="group" aria-label="Editar cantidad">
              <button class="btn btn-outline-secondary" data-action="cart-minus" data-id="${
                it.id
              }">−</button>
              <span class="px-2">${it.qty}</span>
              <button class="btn btn-outline-secondary" data-action="cart-plus"  data-id="${
                it.id
              }">+</button>
            </div>
          </td>
          ...
          <td class="text-end">
            <button class="btn btn-sm btn-link text-danger" data-action="cart-remove" data-id="${
              it.id
            }">x</button>
          </td>
        </tr>
